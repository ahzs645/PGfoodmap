  ...
  <div class="w-[350px] h-full bg-white dark:bg-gray-800 shadow-lg flex flex-col border-r border-gray-200 dark:border-gray-700 z-10">
    ...
    <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-bold text-gray-900 dark:text-white">PG Food Map</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400">Prince George Restaurant Inspections</p>
        </div>
        <!-- Header buttons -->
        <div class="flex items-center gap-2">
          <!-- Roulette button -->
          <button
            @click="emit('open-roulette')"
            class="p-2 rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 transition-all shadow-sm hover:shadow-md"
    ...
    <!-- Stats for hazard mode (at selected date) -->
    <div v-else class="p-4 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
      <div class="text-xs text-gray-500 dark:text-gray-400 mb-2">At Selected Date</div>
      <div class="grid grid-cols-3 gap-2 text-sm">
        <div class="bg-white dark:bg-gray-800 p-2 rounded shadow-sm text-center">
          <div class="text-xl font-bold text-green-600">{{ hazardStatsAtDate?.Low || 0 }}</div>
          <div class="text-xs text-gray-500 dark:text-gray-400">Low</div>
        </div>
        <div class="bg-white dark:bg-gray-800 p-2 rounded shadow-sm text-center">
          <div class="text-xl font-bold text-amber-600">{{ hazardStatsAtDate?.Moderate || 0 }}</div>
          <div class="text-xs text-gray-500 dark:text-gray-400">Moderate</div>
        </div>
        <div class="bg-white dark:bg-gray-800 p-2 rounded shadow-sm text-center">
          <div class="text-xl font-bold text-gray-600">{{ hazardStatsAtDate?.Unknown || 0 }}</div>
